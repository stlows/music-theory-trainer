...
function question() {
  const questionFunc: string = chooseOne(settings.questions);

  if (typeof window[questionFunc] === "function") {
    window[questionFunc]();
  }
}
...
function quelleNoteSurManche() {
  ...
  const fret = random(settings.frets + 1, 1);
...
function createQuestion({ questionText, answerText }) {
  ...
  answer.addEventListener("click", (a: MouseEvent) => {
    if (!questionWrapper.classList.contains("answered")) {
      (a.target as HTMLElement).innerText = answerText;
      questionWrapper.classList.add("answered");
      clearInterval(timeoutId);
    }
  });
    ...
    for (let noteIndex = 0; noteIndex < notes.length; noteIndex++) {
      ...
      if (accordTypesGuitare) {
        ...
        if (accordNotesGuitare) {
          const guitare = createGuitar({ notes: accordNotesGuitare, fretCount: 5 });
