...
function setOption(optionKey, value, isToggle) {
  if (isToggle) {
    ...
    if (settings[optionKey].length === 0) {
      ...
      const element: HTMLElement = document.querySelector(`.settingBtn.${optionKey}:first-child`);
      element.classList.toggle("active");
      settings[optionKey].push(element.dataset.value);
    }
  } else {
    const elements: HTMLElement[] = Array.from(document.querySelectorAll(`.settingBtn.${optionKey}`));
...
function createOptionbutton(optionKey, value, isToggle = false, btnKey = "", className = "", description = "") {
  ...
  button.addEventListener("click", (e) => {
    setOption(optionKey, (e.target as HTMLElement).dataset.value, isToggle);
  });
  return button;
}
