...
let stopContinousQuestionsFlag = false
function stopContinuousQuestions() {
  stopContinousQuestionsFlag = true
  document.querySelector("#newQuestion").style.display = "block"
  document.querySelector("#stopContinuousQuestions").style.display = "none"
}
...
function createGuitarQuestion({ questionText = "Test", notes = [], answerText = "", delayedNotes = false, notTimed = false }) {
  ...
  let timeoutId
  if (!notTimed) {
    timeoutId = addCorrectionAndTimer(questionWrapper, questionWrapper)
  }
...
function addCorrectionAndTimer(questionWrapper, answer, forceCorrectionButtons) {

  let buttonBad
  if (settings.afficherCorrection == "true" || forceCorrectionButtons) {
    let correctionElements = correction()
    buttonBad = correctionElements.buttonBad
    questionWrapper.appendChild(correctionElements.correctionWrapper)
  }
  ...
  if (settings.timerInSeconds > 0) {
    ...
    const timeoutId = setTimeout(() => {
      answer.click()
      if (settings.afficherCorrection == "true" && settings.autoSelectBadAfterTimer === "true") {
        buttonBad.click()
      }
...
function createQuestion({ questionText, answerText, extraInfos, answerNode, light, autoAnswer, notTimed = false, forceCorrectionButtons = false, questionTts, answerTts }) {
  const questionWrapper = div("question")
  if (light) {
  ...
  if (questionTts && settings.ttsQuestion == "true") {
    let utterance = new SpeechSynthesisUtterance(questionTts)
  ...
  let timeoutId = 0
  if (!notTimed) {
    timeoutId = addCorrectionAndTimer(questionWrapper, answer, forceCorrectionButtons)
  }
  ...
  answer.addEventListener("click", (a) => {
    if (!questionWrapper.classList.contains("answered")) {
      ...
      questionWrapper.classList.add("answered")
      clearInterval(timeoutId)
      if (answerTts && settings.ttsQuestion == "true") {
        let utterance = new SpeechSynthesisUtterance(answerTts)
        utterance.lang = "fr-CA"
        utterance.voice = voice
        utterance.onend = checkContinuousQuestion
        window.speechSynthesis.speak(utterance)
      } else {
        checkContinuousQuestion()
      }
...
function intervalle(seededRandom) {
  ...
  createQuestion({
    questionText,
    answerNode,
    questionTts: `${t(intervalle)} ${t("of")} ${noteToSpeech(notes[rootIndex].root)} ?`,
    answerTts: noteToSpeech(notes[rootIndex][intervalle])
...
function nthNoteInKey(seededRandom) {
  ...
  if (seededRandom.next() < 0.5) {
    ...
    questionTts = t("nthNoteInKey")(noteToSpeech(key), romanToSpeech(getRoman(degree)))
    answerTts = noteToSpeech(getChordDegree(key, degree))
  } else {
    questionText = t("whichDegreeInKey")(getChordDegree(key, degree), key)
    answerText = getRoman(degree)
    questionTts = t("whichDegreeInKey")(noteToSpeech(getChordDegree(key, degree)), noteToSpeech(key))
    answerTts = romanToSpeech(getRoman(degree))
  }
  createQuestion({
    questionText,
    answerText,
...
function relativeKey(seededRandom) {
  ...
  let questionText, answerText
  if (seededRandom.next() < 0.5) {
    questionText = t("relativeKey")(key, "m")
    answerText = getChordDegree(key, 6)
    questionTts = t("relativeKey")(noteToSpeech(key), "m")
    answerTts = noteToSpeech(getChordDegree(key, 6))
  } else {
    questionText = t("relativeKey")(getChordDegree(key, 6), "M")
    answerText = key
    questionTts = t("relativeKey")(noteToSpeech(getChordDegree(key, 6)), "M")
    answerTts = noteToSpeech(key)
  }
  createQuestion({
    questionText,
    answerText,
...
function printAllGammes() {
  ...
  const gammeSelect = select(
    [
      ...new Set(
        gammes.map((g) => {
          return { value: g.name, label: t(g.name) }
        })
      ),
    ],
...
function printAllAccords() {
  ...
  const accordSelect = select(
    [
      ...new Set(
        accords.map((a) => {
          return { value: a.name, label: t(a.name) }
        })
      ),
    ],
    { id: "accordSelect" }
...
const midiPianoNotes = [
  19, 21, 23, 24, 26, 28, 29, 31, 33, 35, 36, 38, 40, 41, 43, 45, 47, 48, 50, 52, 53, 55, 57, 59, 60, 62, 64, 65, 67, 69, 71, 72, 74, 76, 77, 79, 81,
...
let octavesDifficuly = {
  easy: { treble: [60, 72], bass: [48, 60] },
  medium: { treble: [55, 81], bass: [43, 62] },
  hard: { treble: [53, 88], bass: [35, 64] },
}
...
function spawnPiano() {
  let { minPiano, maxPiano } = getPianoRange(notesToBePlayed) || { minPiano: "C2", maxPiano: "B6" }
...
function log(msg, type = "success") {
  ...
  const isDebug = params.get("debug") === "true" || settings.debug === "true"
  const fullDebug = params.get("debug") === "full" || settings.debug === "full"
  if (isDebug || fullDebug) {
    console.log(msg)
    if (fullDebug) {
      notify(msg, type)
    }
